...
- **申し送り（後日着手）**: 自社メディア＝柱記事「2023→いま、生成AIにどう取り組んできたか」の年表を**後日ユーザー依頼で作成**（ミッション `2026-0603-own-media-pillar`）。年表の中身データは本人提供待ち。既存 `/media/` は非掲載のまま据え置き、再掲載/新規ページ化は着手時に判断。
...
- **問い合わせフォーム**: フォーム化を**実装済（ローカル検証済・未コミット）**。生メアド撤去＝収集ボット対策（`emailInRenderedDOM:false` を Playwright で確認、フォールバックは JS クリック生成）。仕組み＝**自前ブランドフォーム→Web3Forms（無料・バックエンド無し）**＋**hCaptcha（無料ゼロ設定。Turnstile は Web3Forms では有料のため不採用）**＋honeypot＋高め敷居（種類select＝制作代行/丸投げの選択肢なし＋「いまの状況」必須 minlength30）。プライバシーポリシーに Web3Forms / hCaptcha の外部送信を追記（§4・§5、最終改定2026-06-10）。access key 配線済（ユーザー提供）・コミット済（master、未push）。**残: 本番デプロイ後に1通テスト送信して到達＆迷惑メール振り分けを確認。** 将来 Cloudflare Pages Functions への自前化は保留（手軽さ優先で見送り）。独立レビュー実施（F1対策）。
